...
use crate::core::block::{
    BlockIoError, BlockReadCompletion, BlockReadRequest, BlockSector, BLOCK_SECTOR_SIZE,
};
...
const VIRTIO_VERSION_MMIO_V1: u32 = 1;
...
const PFN_USED_OFFSET: usize = 4096;
...
#[repr(C, align(4096))]
struct PfnQueue([u8; QUEUE_BYTES]);

static mut DESC_TABLE: VirtqDescTable = VirtqDescTable {
    desc: [VirtqDesc::empty(); QUEUE_SIZE],
};
...
static mut USED_RING: VirtqUsed = VirtqUsed {
    flags: 0,
    idx: 0,
    ring: [VirtqUsedElem::empty(); QUEUE_SIZE],
    avail_event: 0,
};
static mut PFN_QUEUE: PfnQueue = PfnQueue([0; QUEUE_BYTES]);
...
static mut PFN_QUEUE_MODE: bool = false;
static mut LAST_USED_IDX: u16 = 0;

pub const fn readiness() -> HardwareReadiness {
    HardwareReadiness::NotReady(ReadinessReason::BlockProviderMissing)
}

pub fn read_sector(request: BlockReadRequest, out: &mut BlockSector) -> BlockReadCompletion {
    match read_sector_inner(request.sector(), out) {
        Ok(()) => BlockReadCompletion::completed(request),
        Err(error) => BlockReadCompletion::failed(request, error),
    }
}

fn read_sector_inner(sector: u64, out: &mut BlockSector) -> Result<(), BlockIoError> {
    ensure_ready()?;
    unsafe {
        ...
        if core::ptr::read_volatile(core::ptr::addr_of!(REQ_STATUS)) != 0 {
            return Err(BlockIoError::DeviceStatusNonZero);
        }

        let mut index = 0usize;
        while index < BLOCK_SECTOR_SIZE {
            out[index] = core::ptr::read_volatile(core::ptr::addr_of!(DATA[index]));
            index += 1;
        }
    }
    Ok(())
...
fn ensure_ready() -> Result<(), BlockIoError> {
    unsafe {
        ...
        if mmio_read(REG_MAGIC) != VIRTIO_MAGIC {
            return Err(BlockIoError::ProviderMissing);
        }
        match mmio_read(REG_VERSION) {
            VIRTIO_VERSION_MODERN => ensure_ready_modern(),
            VIRTIO_VERSION_MMIO_V1 => ensure_ready_mmio_v1(),
            _ => Err(BlockIoError::UnsupportedTransport),
        }
    }
}

unsafe fn ensure_ready_modern() -> Result<(), BlockIoError> {
    if mmio_read(REG_DEVICE_ID) != VIRTIO_DEVICE_BLOCK {
        return Err(BlockIoError::DeviceKindMismatch);
    }
    PFN_QUEUE_MODE = false;
    ...
    if mmio_read(REG_DEVICE_FEATURES) & VIRTIO_F_VERSION_1_HIGH == 0 {
        mmio_write(REG_STATUS, mmio_read(REG_STATUS) | STATUS_FAILED);
        return Err(BlockIoError::FeatureNegotiationFailed);
    }
...
unsafe fn ensure_ready_mmio_v1() -> Result<(), BlockIoError> {
    if mmio_read(REG_DEVICE_ID) != VIRTIO_DEVICE_BLOCK {
        return Err(BlockIoError::DeviceKindMismatch);
    }
    PFN_QUEUE_MODE = true;
    ...
    let base = core::ptr::addr_of_mut!(PFN_QUEUE) as *mut u8;
    let mut index = 0usize;
    while index < QUEUE_BYTES {
...
unsafe fn desc_ptr() -> *mut VirtqDesc {
    if PFN_QUEUE_MODE {
        core::ptr::addr_of_mut!(PFN_QUEUE) as *mut VirtqDesc
    } else {
...
unsafe fn avail_ptr() -> *mut VirtqAvail {
    if PFN_QUEUE_MODE {
        (core::ptr::addr_of_mut!(PFN_QUEUE) as *mut u8)
...
unsafe fn used_ptr() -> *mut VirtqUsed {
    if PFN_QUEUE_MODE {
        (core::ptr::addr_of_mut!(PFN_QUEUE) as *mut u8).add(PFN_USED_OFFSET) as *mut VirtqUsed
    } else {

...
const RUN_COMMANDS: &[BusyboxCommand] = &[
    ...
    BusyboxCommand {
        name: "printf",
        argv: &[b"busybox", b"printf", b"abc\\n"],
        expected_exit: 0,
        official_name: Some("printf \"abc\\n\""),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "uname",
    ...
    BusyboxCommand {
        name: "dirname",
        argv: &[b"busybox", b"dirname", b"/aaa/bbb"],
        expected_exit: 0,
        official_name: Some("dirname /aaa/bbb"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "expr",
        argv: &[b"busybox", b"expr", b"1", b"+", b"1"],
        expected_exit: 0,
        official_name: Some("expr 1 + 1"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "date",
        argv: &[b"busybox", b"date"],
        expected_exit: 0,
        official_name: Some("date"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "uptime",
        argv: &[b"busybox", b"uptime"],
        expected_exit: 0,
        official_name: Some("uptime"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "clear",
        argv: &[b"busybox", b"clear"],
        expected_exit: 0,
        official_name: Some("clear"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "cal",
        argv: &[b"busybox", b"cal"],
        expected_exit: 0,
        official_name: Some("cal"),
        class: BusyboxCommandClass::Scoring,
    },
    BusyboxCommand {
        name: "ls",
...
const DISABLED_COMMANDS: &[BusyboxCommand] = &[
    ...
    BusyboxCommand {
        name: "which",
        argv: &[b"busybox", b"which", b"ls"],
        expected_exit: 0,
        official_name: Some("which ls"),
        class: BusyboxCommandClass::Disabled,
    },
    BusyboxCommand {
        name: "hwclock",
    ...
    BusyboxCommand {
        name: "sleep",
        argv: &[b"busybox", b"sleep", b"1"],
        expected_exit: 0,
        official_name: Some("sleep 1"),
        class: BusyboxCommandClass::Disabled,
    },
];

#[cfg(loongarch64_busybox_diag)]
const DIAGNOSTIC_TICK_LIMIT: usize = 64;

#[cfg(loongarch64_busybox_diag)]
const DIAGNOSTIC_COMMANDS: &[BusyboxCommand] = &[
    BusyboxCommand {
        name: "diag-basename",
        argv: &[b"busybox", b"basename", b"/aaa/bbb"],
        expected_exit: 0,
...
#[cfg(loongarch64_busybox_diag)]
pub(crate) fn run_loongarch_busybox_diagnostics() {
    early_console_write("[loongarch64-busybox-diag] non-scoring diagnostics begin\n");
    let mut i = 0usize;
    while i < DIAGNOSTIC_COMMANDS.len() {
        run_diagnostic_command(&DIAGNOSTIC_COMMANDS[i]);
        i += 1;
    }
    early_console_write("[loongarch64-busybox-diag] non-scoring diagnostics end\n");
}
...
#[cfg(loongarch64_busybox_diag)]
fn run_diagnostic_command(command: &BusyboxCommand) {
    let mut argv = [ExecString::empty(); 8];
    if command.argv.len() > argv.len() {
        early_console_write("[loongarch64-busybox-diag] blocker: too many argv entries command=");
        early_console_write(command.name);
        early_console_write("\n");
        return;
    }
    ...
    while i < command.argv.len() {
        if argv[i].set_from_slice(command.argv[i]).is_err() {
            early_console_write("[loongarch64-busybox-diag] blocker: argv setup failed command=");
            early_console_write(command.name);
            early_console_write("\n");
            return;
        }
    ...
    match real_elf::load_user_elf_with_args("/musl/busybox", &argv[..command.argv.len()], &[]) {
        Ok(load) => {
            early_console_write("[loongarch64-busybox-diag] load command=");
            early_console_write(command.name);
            early_console_write(" entry=");
            write_usize_hex(load.entry);
            early_console_write(" first_load=");
            ...
            write_usize_dec(load.load_segments);
            early_console_write("\n");
            run_diagnostic_probe(command.name, load.entry, load.stack_pointer);
        }
        Err(err) => {
            early_console_write("[loongarch64-busybox-diag] blocker: failed to load /musl/busybox: ");
            early_console_write(err);
            early_console_write(" command=");
            early_console_write(command.name);
            early_console_write("\n");
...
#[cfg(loongarch64_busybox_diag)]
fn run_diagnostic_probe(command_name: &str, entry: usize, stack_pointer: usize) {
    user::reset_case_state();
    user::start_syscall_budget(1024);
    let mut cwd = PathBuf::empty();
    let _ = cwd.set_from_slice(b"/musl");
    fd_table::set_cwd(&cwd);

    early_console_write("[loongarch64-busybox-diag] command-start=");
    early_console_write(command_name);
    early_console_write(" entry=");
    write_usize_hex(entry);
    early_console_write("\n");
    match real_elf::activate_current_user_mmu() {
        Ok(()) => {
            early_console_write("[loongarch64-busybox-diag] mapped entry=");
            write_usize_hex(real_elf::current_entry());
            early_console_write("\n");
        }
        Err(err) => {
            early_console_write("[loongarch64-busybox-diag] blocker: failed to activate user mmu: ");
            early_console_write(err);
            early_console_write(" command=");
            early_console_write(command_name);
            early_console_write("\n");
    ...
    user::start_diagnostic_run(DIAGNOSTIC_TICK_LIMIT);
    trap::start_diagnostic_timer();
    trap::enter_user_entry(entry, stack_pointer);
    trap::stop_diagnostic_timer();
    user::finish_diagnostic_run();
    real_elf::deactivate_current_user_mmu();

    let report = user::run_snapshot();
    emit_diagnostic_summary(command_name, entry, &report);
    if report.fault_active {
        real_elf::dump_user_regions("[loongarch64-busybox-diag] ");
    }
}

#[cfg(loongarch64_busybox_diag)]
fn emit_diagnostic_summary(
    command_name: &str,
    entry: usize,
    report: &user::UserRunSnapshot,
) {
    early_console_write("[loongarch64-busybox-diag] result command=");
    early_console_write(command_name);
    early_console_write(" status=");
    if report.exited {
        early_console_write("exited exit_code=");
        write_usize_dec(report.exit_code);
    } else if report.timeout_active {
        early_console_write("timeout");
    } else if report.fault_active {
        early_console_write("fault");
    } else if report.missing_syscall_active {
        early_console_write("missing-syscall id=");
        write_usize_dec(report.missing_syscall_id);
    } else {
        early_console_write("returned-without-exit");
    }
    early_console_write(" entry=");
    write_usize_hex(entry);
    early_console_write(" last_era=");
    write_usize_hex(report.diagnostic_last_era);
    early_console_write(" last_badv=");
    write_usize_hex(report.diagnostic_last_badv);
    early_console_write(" last_ecode=");
    write_usize_dec(report.diagnostic_last_ecode);
    early_console_write(" syscalls=");
    write_usize_dec(report.timeout_syscalls);
    early_console_write(" traps=");
    write_usize_dec(report.diagnostic_trap_count);
    early_console_write(" same_era=");
    write_usize_dec(report.diagnostic_same_era_count);
    early_console_write(" timer_ticks=");
    write_usize_dec(report.diagnostic_timer_ticks);
    early_console_write(" stopped_before_syscall=");
    if report.timeout_active && report.timeout_syscalls == 0 {
        early_console_write("yes");
    } else {
        early_console_write("no");
    }
    if report.timeout_last_syscall_id != 0 {

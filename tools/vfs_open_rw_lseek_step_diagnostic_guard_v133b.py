...
print("[INFO] VFS step diagnostic guard v133b started")
...
tokens = [
    "SYS_OPENAT", "SYS_WRITE", "SYS_LSEEK", "SYS_READ", "SYS_CLOSE",
    "FAIL step=openat", "FAIL step=write", "FAIL step=lseek", "FAIL step=read", "FAIL step=compare",
    "vfs_open_rw_lseek_v133b.txt", "vfs-roundtrip-133"
]
for token in tokens:
    if token not in builder_text:
        fail(f"diagnostic builder missing token: {token}")
ok("diagnostic external init builder contains step-specific markers")

init_elf = ROOT / "user/init.elf"
if not init_elf.exists():
    fail("user/init.elf missing")
blob = init_elf.read_bytes()
for token in [b"hello from external init.elf v133b syscall write", b"FAIL step=openat", b"FAIL step=write", b"FAIL step=lseek", b"FAIL step=read", b"FAIL step=compare"]:
    if token not in blob:
        fail(f"user/init.elf missing embedded diagnostic token: {token!r}")
ok("user/init.elf embeds step diagnostic tokens")

coverage = {}
for name, needles in {
    "VFS dispatch": ["SYS_OPENAT", "SYS_WRITE", "SYS_LSEEK", "SYS_READ", "SYS_CLOSE"],
    "VFS helpers": ["openat", "write", "lseek", "read", "close"],
    "state/error": ["fd", "offset", "pos", "EFAULT", "EBADF", "EINVAL"],
}.items():
...
manifest_path = out_dir / "vfs_open_rw_lseek_step_diagnostic_manifest_v133b.json"
manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True))
print(f"[INFO] manifest: {manifest_path}")
ok("VFS step diagnostic guard v133b completed")

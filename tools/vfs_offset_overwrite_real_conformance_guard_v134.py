...
print("[INFO] VFS offset-overwrite real conformance guard v134 started")
...
for name, needles in {
    ...
    "offset overwrite semantics vocabulary": ["offset", "pos", "seek", "size", "len"],
...
for token in ["SYS_OPENAT", "SYS_WRITE", "SYS_LSEEK", "SYS_READ", "SYS_CLOSE", "abcdef", "XYZ", "abcXYZ", "vfs_offset_overwrite PASS"]:
    if token not in builder_text:
        fail(f"builder missing token: {token}")
ok("external init builder contains offset-overwrite syscall sequence")

init_elf = ROOT / "user/init.elf"
if not init_elf.exists():
    fail("user/init.elf missing")
blob = init_elf.read_bytes()
for token in [b"hello from external init.elf v134 syscall write", b"[ucompat-v134] vfs_offset_overwrite PASS", b"abcdef", b"XYZ", b"abcXYZ"]:
    if token not in blob:
        fail(f"user/init.elf missing embedded token: {token!r}")
ok("user/init.elf embeds offset-overwrite markers and payloads")

manifest = {
    "version": VERSION,
    "phase": "vfs-offset-overwrite-real-conformance",
    "runtime_required_pass_marker": "[ucompat-v134] vfs_offset_overwrite PASS",
    "scenario": "openat -> write abcdef -> lseek 3 -> write XYZ -> lseek 0 -> read -> compare abcXYZ -> close",
    "coverage": coverage,
...
manifest_path = out_dir / "vfs_offset_overwrite_real_conformance_manifest_v134.json"
manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True))
print(f"[INFO] manifest: {manifest_path}")
ok("VFS offset-overwrite real conformance guard v134 completed")

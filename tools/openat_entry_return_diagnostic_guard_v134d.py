...
print("[INFO] openat entry/return diagnostic guard v134d started")
...
for token in ["PRE", "RETURN_PASS", "RETURN_FAIL", "t1_prev_v133b_create_trunc"]:
    if token not in builder_text:
        fail(f"builder missing diagnostic token: {token}")
ok("builder contains PRE/RETURN openat diagnostic")

init_elf = ROOT / "user/init.elf"
blob = init_elf.read_bytes()
for token in [b"hello from external init.elf v134d syscall write", b"[ucompat-v134d] openat_entry", b"PRE", b"RETURN_PASS", b"RETURN_FAIL"]:
    if token not in blob:
        fail(f"user/init.elf missing embedded token: {token!r}")
ok("user/init.elf embeds openat entry/return diagnostic markers")

manifest = {
    "version": VERSION,
    "phase": "openat-entry-return-diagnostic",
    "diagnostic_prefix": "[ucompat-v134d] openat_entry",
...
manifest_path = out_dir / "openat_entry_return_diagnostic_manifest_v134d.json"
manifest_path.write_text(json.dumps(manifest, indent=2, sort_keys=True))
print(f"[INFO] manifest: {manifest_path}")
ok("openat entry/return diagnostic guard v134d completed")
